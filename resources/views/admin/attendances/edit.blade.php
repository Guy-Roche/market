...
@endsection
@section('content')
    <link rel="stylesheet" href="{{ asset('backend/css/attend.css') }}">
    <style>
        .switch-toggle {
            width: auto;
        }
        ...
        <div class="container-fluid">
                    ...
                    <div class="page-title-box">
                        <div class="page-title-right">
                            <ol class="breadcrumb m-0">
                                <h4>
                                    <a href="{{ route('attendances') }} " class="btn btn-blue float-sm-right"> <i
            ...
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <div class="card-body">
                            <form action="{{ route('attendance.save') }}" method="post" id="myForm">
                                @csrf
                                        ...
                                        @foreach ($attendances as $key => $attendance)
                                            <tr id="div {{ $attendance->id }}" class="text-center">
                                                <input type="hidden" name="employee_id[]"
                                                    value="{{ $attendance->employee_id }}" class="employee_id">
                                                <td>{{ $key + 1 }}</td>
                                                ...
                                                <td colspan="3">
                                                    <div class="switch-toggle switch-3 switch-candy">
                                                        <input class="present" id="present{{ $key }}"
                                                            ...
                                                            type="radio" {{ $attendance->attend_status == 'Present' ? 'checked' : '' }}>
                                                        <label for="present{{ $key }}">Present</label>
                                                        
                                                        <input class="leave" id="leave{{ $key }}"
                                                            name="attend_status{{ $key }}" value="Leave"
                                                            type="radio" {{ $attendance->attend_status == 'Leave' ? 'checked' : '' }}>
                                                        <label for="leave{{ $key }}">Leave</label>

                                                        <input class="absent" id="absent{{ $key }}"
                                                            name="attend_status{{ $key }}" value="Absent"
                                                            type="radio" {{ $attendance->attend_status == 'Absent' ? 'checked' : '' }}>
